...
const SearchBar = (placeholder: any): JSX.Element => {
    ...
    const handleFilter = (event: any) => {
        ...
        const newFilter = data.allMdx.nodes.filter((value: any) => {
          return value?.frontmatter.title.toLowerCase().includes(searchWord.toLowerCase());
        });
        ...
        <div className='search'>
            ...
            <section id='list' className="dataResult rounded-1xl">
                {filteredData.map((values: any) => {
                    return (<a className='dataItem' href={'/docs'+values.frontmatter.slug}><p>{values?.frontmatter.title}</p></a>)
                })}
            </section>
        </div>
        </>
...
const PopUp = (props: any): any => {
    return (props.trigger) ? (
        <section className='popup'>
            <section>
                <button className='closeBtn' onClick={() => props.setTrigger(false)}><SearchIcon icon={<RiCloseFill />} /></button>
